...
static Bitu INT15_Handler(void) {
	...
	case 0x41:	/* Wait for External Event */ // for PS/55
		CALLBACK_SCF(false);
		break;
...
public:
	BIOS(Section* configuration):Module_base(configuration){
		...
		if (IS_PS55_ARCH) {//for PS/55
			...
			FE6F2h
			int 15h ax=c501h (INT 19h) Bootstrap Loader	->callback[10]
			FE739h
			int 15h ax=c502h (INT 14h) Serial port		->callback[3]
			FE82Eh
			int 15h ax=c503h (INT 16h) Keyboard
			FEC59h
			int 15h ax=c504h (INT 40h) Diskette
			FEFD2h
			int 15h ax=c505h (INT 17h) Printer			->callback[5]	(*J4.0)
			FF065h
			int 15h ax=c506h (INT 10h) Video							(*J4.0)
			FF841h
			int 15h ax=c507h (INT 12h) Get memory size	->callback[2]
			FF84Dh
			int 15h ax=c508h (INT 11h) Get equipment list->callback[1]
			FFE6Eh
			int 15h ax=c509h (INT 1Ah) Time				->callback[6]	(*K3.3)
			*/
			...
			//rptr = CALLBACK_RealPointer(call_10);		//INT 10h Video (used by DOS J4.0?)
			//phys_writeb(0xFF065 + 0, 0xEA);				// FARJMP
			//phys_writew(0xFF065 + 1, RealOff(rptr));	// offset
			//phys_writew(0xFF065 + 3, RealSeg(rptr));	// segment
