...
VideoModeBlock ModeList_PS55[] = {
	/*
	--- PS/55 video modes ---
	Hex 3: 80x25 VGA Mode-3 compatible (Undocumented?)
	Hex 8: 80x25 Monochrome Character
	Hex A: 1024x768 Monochrome Graphics
	Hex E: 80x25 Color Character
	Hex D: 1024x768 16 Color Graphics
...
//static Bit8u ps55_mono_mono[64][3] = //for IBMJ Display Adapter with monochrome monitor (unused)
//{//for PS/55
//	{0x00,0x00,0x00}, {0x05,0x05,0x05}, {0x19,0x19,0x19}, {0x1D,0x1D,0x1D}, {0x0D,0x0D,0x0D}, {0x11,0x11,0x11}, {0x25,0x25,0x25}, {0x2A,0x2A,0x2A},
//	{0x02,0x02,0x02}, {0x07,0x07,0x07}, {0x1B,0x1B,0x1B}, {0x20,0x20,0x20}, {0x0F,0x0F,0x0F}, {0x14,0x14,0x14}, {0x28,0x28,0x28}, {0x2C,0x2C,0x2C},
//	{0x0C,0x0C,0x0C}, {0x11,0x11,0x11}, {0x25,0x25,0x25}, {0x2A,0x2A,0x2A}, {0x19,0x19,0x19}, {0x1E,0x1E,0x1E}, {0x32,0x32,0x32}, {0x36,0x36,0x36},
//	{0x0F,0x0F,0x0F}, {0x13,0x13,0x13}, {0x27,0x27,0x27}, {0x2C,0x2C,0x2C}, {0x1B,0x1B,0x1B}, {0x20,0x20,0x20}, {0x34,0x34,0x34}, {0x39,0x39,0x39},
//	{0x06,0x06,0x06}, {0x0B,0x0B,0x0B}, {0x1F,0x1F,0x1F}, {0x24,0x24,0x24}, {0x13,0x13,0x13}, {0x18,0x18,0x18}, {0x2C,0x2C,0x2C}, {0x30,0x30,0x30},
//	{0x09,0x09,0x09}, {0x0D,0x0D,0x0D}, {0x21,0x21,0x21}, {0x26,0x26,0x26}, {0x15,0x15,0x15}, {0x1A,0x1A,0x1A}, {0x2E,0x2E,0x2E}, {0x33,0x33,0x33},
//	{0x13,0x13,0x13}, {0x17,0x17,0x17}, {0x2B,0x2B,0x2B}, {0x30,0x30,0x30}, {0x1F,0x1F,0x1F}, {0x24,0x24,0x24}, {0x38,0x38,0x38}, {0x3D,0x3D,0x3D},
//	{0x15,0x15,0x15}, {0x1A,0x1A,0x1A}, {0x2E,0x2E,0x2E}, {0x32,0x32,0x32}, {0x22,0x22,0x22}, {0x26,0x26,0x26}, {0x3A,0x3A,0x3A}, {0x3F,0x3F,0x3F}
//};
static Bit8u ps55_palette_color[64][3] = //for IBMJ Display Adapter with color monitor
{//for PS/55
{0x00,0x00,0x00},{0x00,0x00,0x2A},{0x00,0x2A,0x00},{0x00,0x2A,0x2A},{0x2A,0x00,0x00},{0x2A,0x00,0x2A},{0x2A,0x2A,0x00},{0x2A,0x2A,0x2A},
{0x00,0x00,0x15},{0x00,0x00,0x3F},{0x00,0x2A,0x15},{0x00,0x2A,0x3F},{0x2A,0x00,0x15},{0x2A,0x00,0x3F},{0x2A,0x2A,0x15},{0x2A,0x2A,0x3F},
{0x00,0x15,0x00},{0x00,0x15,0x2A},{0x00,0x3F,0x00},{0x00,0x3F,0x2A},{0x2A,0x15,0x00},{0x2A,0x15,0x2A},{0x2A,0x3F,0x00},{0x2A,0x3F,0x2A},
...
static Bit8u ps55_attr_mono[16] = //for video mode 08h monochrome character
{//for PS/55
	0x00,0x00,0x12,0x3f,0x12,0x12,0x00,0x00,0x12,0x3f,0x00,0x00,0x12,0x12,0x00,0x00
};

static Bit8u ps55_attr_color[16] = //for video mode 0eh color character
{//for PS/55
	...
	if (GCC_UNLIKELY(CurMode->mode!=bios_mode)) {
		...
		case VGA_ARCH_CASE:
			...
			case SVGA_ParadisePVGA1A:
				...
				break;
			case SVGA_PS55://for PS/55
				if(ps55.carden) mode_changed = SetCurMode(ModeList_PS55, bios_mode);
				else mode_changed = SetCurMode(ModeList_VGA, bios_mode);
				break;
			case SVGA_S3Trio:
...
static void FinishSetMode(bool clearmem) {
	...
	real_writew(BIOSMEM_SEG,BIOSMEM_CRTC_ADDRESS,((CurMode->mode==7 )|| (CurMode->mode==0x0f)) ? 0x3b4 : 0x3d4);/*r4467
	...
	if (IS_EGAVGA_ARCH) {
		...
		/* Set font pointer */
		if ((isJEGAEnabled()) && (CurMode->mode == 0x03 || CurMode->mode == 0x53)) //for AX
	...
	else if (CurMode->type == M_PS55_TEXT)//for PS/55
	{
		real_writeb(BIOSMEM_SEG, BIOSMEM_VIDEO_CTL, (0x61 | (clearmem ? 0 : 0x80)));//disable VGA cursor emulation
		INT10_SetCursorShape(0x1b, 0x1c);
	}
	...
	/* Set some interrupt vectors *//*r4467
...
//	Bit8u vga_switches=real_readb(BIOSMEM_SEG,BIOSMEM_SWITCHES);
	...
	if (IS_AX_ARCH) {
		...
		LOG(LOG_INT10, LOG_NORMAL)("EGA:JEGA Display Out is disabled.");
	}
	...
	if (INT10_AX_GetCRTBIOSMode() == 0x51) {
		...
		if (mode == 0x03 || mode == 0x53)
		{
			...
			IO_WriteB(0x3d5, tmp);
			real_writeb(BIOSMEM_AX_SEG, BIOSMEM_AX_JEGA_RMOD1, tmp);;
			LOG(LOG_INT10, LOG_NORMAL)("EGA:JEGA Display Out is enabled.");
		}
		...
		case SVGA_PS55://for PS/55
			if (ps55.carden) {
				if (!SetCurMode(ModeList_PS55, mode)) {
					LOG(LOG_INT10, LOG_ERROR)("VGA:Trying to set illegal mode %X", mode);
					return false;
				}
			}
...
/*	if ((CurMode->type==M_TEXT) && (CurMode->cwidth==9)) {
	...
	case M_LIN32:
		...
		break;
	case M_PS55_TEXT:
 		offset = CurMode->hdispend;//is this correct?
		break;
	default:
	...
	case M_TANDY16:
		...
		break;
	case M_PS55_TEXT://for PS/55
		att_data[0x10] = 0x00;		//Set Attr Mode Reg
		IO_WriteW(0x3e8, 0x113b);//Set Cursor Options
		if (CurMode->mode == 0xe) {
			IO_WriteW(0x3e8, 0x803d);//Set Attribute Mode: Color
			IO_WriteW(0x3e8, 0x0734);//Set Line Color: 07 (Black)
			//IO_WriteW(0x3e8, 0x0f3a);//Cursor Color (set by INT10_SetCursorShape)
			for (i = 0; i < 16; i++) {
		...
		{
			IO_WriteW(0x3e8, 0x003d);//Set Attribute Mode: Mono
			IO_WriteW(0x3e8, 0x0234);//Set Line Color: 02 (Green)
			//IO_WriteW(0x3e8, 0x013a);//Cursor Color (set by INT10_SetCursorShape)
			for (i = 0; i < 16; i++) {
	...
	if ((modeset_ctl & 8)==0) {
		...
		case M_PS55_TEXT:
			for (i = 0; i < 256; i++) {
				...
				IO_Write(0x3c9, ps55_palette_color[i&0x3F][2]);
			}
			break;
		case M_TEXT:
			if (CurMode->mode==7) {
...
dac_text16:
	...
	/* Setup some special stuff for different modes *//*r4467
...
//	real_writeb(BIOSMEM_SEG,BIOSMEM_INITIAL_MODE,feature);
	...
	else if (CurMode->type == M_PS55_TEXT) {
		//BIOS feature not available
		IO_WriteW(0x3e0, 0x1008);//Turn on font memory access
		IO_WriteW(0x3e2, 0x8008);//?
		IO_WriteW(0x3e2, 0xb00b);//select Gaiji RAM
		IO_WriteW(0x3e2, 0x010a);//select Bank 1
		PhysPt fontbase = PhysMake(0xb000, 0x4000);
		for (Bitu chr = 0; chr < 256; chr++) {
			if (!isKanji1(chr)) {
				//put 12 x 24 character font into 16 x 30 memory block
				mem_writew(fontbase + chr * 64 + 0, 0x0000);
				mem_writew(fontbase + chr * 64 + 2, 0x0000);
				mem_writew(fontbase + chr * 64 + 52, 0x0000);
				mem_writew(fontbase + chr * 64 + 54, 0x0000);
				mem_writew(fontbase + chr * 64 + 56, 0x0000);
				mem_writew(fontbase + chr * 64 + 58, 0x0000);
				mem_writew(fontbase + chr * 64 + 60, 0x0000);
				mem_writew(fontbase + chr * 64 + 62, 0x0000);
				for (Bitu line = 0; line < 32; line++)
				{
					Bit16u font = ps55font_24[0x98000 + chr * 64 + line * 2 + 1];
					font <<= 8;
					font |= ps55font_24[0x98000 + chr * 64 + line * 2];
					//mem_writew(fontbase + chr * 64 + 4 + line * 2, font);
					mem_writew(fontbase + chr * 64 + line * 2, font);
				}
			}
		}
		IO_WriteW(0x3e0, 0x0008);//Turn off font memory access
	}
	return true;
}
...
Bitu VideoModeMemSize(Bitu mode) {
	...
	case SVGA_ParadisePVGA1A:
		...
		break;
	case SVGA_PS55:
		if(ps55.carden) modelist = ModeList_PS55;
		else modelist = ModeList_VGA;
		break;
	default:
