...
{	"MODE",		0,			&DOS_Shell::CMD_MODE,		"SHELL_CMD_MODE_HELP"},//for PS/55
...
void DOS_Shell::CMD_CLS(char * args) {
	HELP("CLS");
	reg_ax=0x0003;
	//if (svgaCard == SVGA_PS55) reg_ax = 0x0008;//for PS/55
...
void DOS_Shell::CMD_MODE(char* args) {
	char buffer[512];
	char* pbuffer = buffer;
	Bit8u set_videomode = 0;
	safe_strncpy(buffer, args, 512);
	StripSpaces(pbuffer);
	if (!*args) {
		WriteOut("Current video mode is %X", real_readb(0x40, 0x49));
		return;
	}
