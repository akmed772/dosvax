...
public:
	Bitu readHandler(PhysPt start) {
		...
		//if (start % 0x80 < 6) 
		//	LOG_MSG("x: %04d y: %03d vga.latch.d: %08X (plane %d: %02X)", start % 0x80 * 8, start / 0x80, vga.latch.d, vga.config.read_map_select, vga.latch.b[vga.config.read_map_select]);
...
INLINE static Bit32u ModeOperation2(Bit8u val) {
	Bit32u full;
	switch (vga.config.write_mode) {
	case 0x00:
		// Write Mode 0:
...
class VGA_UnchainedPS55_Handler : public VGA_UnchainedRead_Handler {
	void writeData(PhysPt start, Bit8u val) {
		...
		*(Bit32u*)(write_pixels + 4) = colors4_7;
		...
		// for 256 color support TODO: must support many unknown regs I/O 3ED-3EFh
		//vga.latch.d = ((Bit32u*)vga.mem.linear)[start + 0x20000];
		////data = ModeOperation2(val);
		///* Update video memory and the pixel buffer */
		////VGA_Latch pixels, debug_dest;
		//pixels.d = ((Bit32u*)vga.mem.linear)[start + 0x20000];
		//debug_dest.d = pixels.d;
		//pixels.d &= ps55.full_not_map_mask_high;
		//pixels.d |= (data & ps55.full_map_mask_high);
		//((Bit32u*)vga.mem.linear)[start + 0x20000] = pixels.d;
		//write_pixels = &vga.fastmem[(start << 3)];
		//debug_destpx1 = *(Bit32u*)write_pixels;
		//debug_destpx2 = *(Bit32u*)(write_pixels + 4);
		////Bit32u colors0_3, colors4_7;
		...
		//*(Bit32u*)(write_pixels + 4) |= colors4_7;
		////for heeaavy debug
		//colors0_3 = *(Bit32u*)write_pixels;
		//colors4_7 = *(Bit32u*)(write_pixels + 4);
		//if ((start % 0x80 < 10) && (start < 0x7400)) {
		//	LOG_MSG("H x %04d y %03d mode%X rop%X src %02X smsk %08X bmsk %08X dst %08X lat %08X px %08X %08X",
		//		start % 0x80 * 8, start / 0x80, vga.config.write_mode, vga.config.raster_op, val, ps55.full_enable_and_set_reset_high, vga.config.full_bit_mask, vga.latch.d, data, debug_destpx1, debug_destpx2);
		//	LOG_MSG("H x %04d y %03d mmsk %02X rd %08X, wt %08X,                                       px %08X %08X",
		//		start % 0x80 * 8, start / 0x80, ps55.map_mask, debug_dest.d, pixels.d, colors0_3, colors4_7);
		//}
	}
public:
	void writeHandler(PhysPt start, Bit8u val) {
		...
		//vga.config.full_map_mask = ps55.full_not_map_mask_high;
		writeData(start, val);
	}
...
public:
	...
	void writew(PhysPt addr, Bitu val) {
		...
		addr = CHECKED2(addr);
		MEM_CHANGED(addr << 3);
		writeHandlerW(addr + 0, (Bit16u)(val));
	}
	//void writed(PhysPt addr, Bitu val) {
		//addr = PAGING_GetPhysicalAddress(addr) & vgapages.mask;
		//addr += vga.svga.bank_write_full;
...
static struct vg {
	...
	VGA_UnchainedPS55_Handler	u5ga;
...
//		MEM_SetPageHandler(vga.tandy.mem_bank<<2,vga.tandy.is_32k_mode ? 0x08 : 0x04,range_handler);
	...
	case M_PS55_GFX:
		vgapages.base = VGA_PAGE_A0;
		vgapages.mask = 0x1ffff;
		//vga.svga.bank_read_full = 0xff00;
		//vga.svga.bank_read_full = (Bit32u)ps55.crtc_reg[0x0c] << 8;
		//vga.svga.bank_write_full = vga.svga.bank_read_full;
		MEM_ResetPageHandler(PS55_BASE_E0, 1);
		MEM_SetPageHandler(VGA_PAGE_A0, 32, &vgaph.u5ga);
		goto range_done;
		break;
	case M_PS55_GFX_MONO:
		...
		break;
	case M_PS55_TEXT:
		//vga.gfx.miscellaneous &= 0xfd;
		vgapages.base = PS55_BASE_E0;
		vgapages.mask = 0x0fff;
		...
		MEM_SetPageHandler(PS55_BASE_E0, 1, &vgaph.map);
		goto range_done;
		break;
	case M_PS55_M3TEXT:
		vgapages.base = VGA_PAGE_B0;
