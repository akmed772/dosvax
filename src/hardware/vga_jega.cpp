...
void write_p3d5_jega(Bitu reg, Bitu val, Bitu iolen) {
	...
	case 0xbf://font r/w register
		...
		break;
	default:
		LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:GFX:JEGA:Write to illegal index %02X", reg);
		break;
	}
}
...
Bitu read_p3d5_jega(Bitu reg, Bitu iolen) {
	...
	case 0xdf:
		...
		break;
	case 0xbf:
		ret = 0x03;//Font always read/writeable
		break;
	default:
		LOG(LOG_VGAMISC, LOG_NORMAL)("VGA:GFX:JEGA:Read from illegal index %02X", reg);
		break;
	}
#if C_HEAVY_DEBUG
