...
static void write_p60(Bitu /*port*/,Bitu val,Bitu /*iolen*/) {
	...
	case CMD_SELSCANCODES:
		...
		//	keyb.repeat.rate = 33;
		//	KEYBOARD_AddBuffer(0xfa);	/* Acknowledge */
		//	LOG(LOG_KEYBOARD, LOG_NORMAL)("Switched to default scancode set");
		//}
		else KEYBOARD_AddBuffer(0xfe); /* Resend */
...
/*			KEYBOARD_AddBuffer(0xe0);
		...
		case KBD_backslash:ret = 0x0b; break;//for AX layout
		case KBD_yen:ret = 0x30; break;//for AX layout
		case KBD_conv:ret = 0x35; break;//for AX layout
		case KBD_nconv:ret = 0x33; break;//for AX layout
	...
	else
	{
		...
		case KBD_printscreen:
			...
			//case KBD_underscore:ret = 0x73; break;//for JP layout
			//case KBD_yen:ret = 0x7d; break;//for JP layout
		case KBD_backslash:ret = 86; break;//for AX layout
		...
		case KBD_kpcomma:extend = true; ret = 51; break;//scancode set 01 for PS/55
		default:
			E_Exit("Unsupported key press");
			break;
		}
