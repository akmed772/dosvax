...
bool device_CON::Read(Bit8u * data,Bit16u * size) {
	...
	INT10_SetCurMode();
	Bit8u writeAttr = 7;
	if (IS_MODE_PS55TEXT) writeAttr = 0;//for PS/55
	if ((readcache) && (*size)) {
		data[count++]=readcache;
		if(dos.echo) INT10_TeletypeOutput(readcache, writeAttr);
		readcache=0;
	}
	while (*size>count) {
		...
		case 13:
			...
			*size=count;
			reg_ax=oldax;
			if(dos.echo) { 
				INT10_TeletypeOutput(13, writeAttr); //maybe don't do this ( no need for it actually ) (but it's compatible)
				INT10_TeletypeOutput(10, writeAttr);
			}
			return true;
			break;
		case 8:
			if(*size==1) data[count++]=reg_al;  //one char at the time so give back that BS
				...
				data[count--]=0;
				INT10_TeletypeOutput(8, writeAttr);
				INT10_TeletypeOutput(' ', writeAttr);
			} else {
		...
		if(dos.echo) { //what to do if *size==1 and character is BS ?????
			INT10_TeletypeOutput(reg_al, writeAttr);
		}
	}
	*size=count;
	reg_ax=oldax;
	...
	switch(data[count]){
		...
		case 'm':               /* SGR */
			for(i=0;i<=ansi.numberofarg;i++){ 
				if (IS_MODE_PS55TEXT) {//for PS/55
					switch (ansi.data[i]) {
					case 0: /* normal */
						ansi.attr = 0x00;//Real ansi does this as well. (should do current defaults)
						break;
					case 1: /* highlight */
						ansi.attr |= 0x08;
						break;
					...
					case 30: /* fg color black */
						ansi.attr &= 0x37;//MSB BGxxRxxx LSB -> 00110111
						ansi.attr |= 0xc8;
						break;
...
void device_CON::Output(Bit8u chr) {
	...
	} else INT10_TeletypeOutput(chr, writeAttr);
 }
