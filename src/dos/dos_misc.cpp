...
/*
INT 1Fh Get the pointer to the device configuration block (Japanese DOS only?)
Input:
	AX = index
Return:
	ES(CX):BX = Pointer to the device configuration block
	CX = 0 if the index is invalid
*/
static Bitu INT1F_Handler(void) {
	switch (reg_ax) {
	case 0x08:
		reg_cx = RealSeg(dos.tables.dcb_disp);
		SegSet16(es, reg_cx);
		reg_bx = RealOff(dos.tables.dcb_disp);
		break;
	default:
		reg_cx = 0;
	}
...
void DOS_SetupMisc(void) {
	...
	if (IS_PS55_ARCH) {
		//overwrite Intertrupt Vector 1Fh
