...
void DOS_SetupTables(void) {
	...
	/* Allocate Device Configuration Block (for PS/55) */
	if (IS_PS55_ARCH) {
		dos.tables.dcb_disp = RealMake(DOS_GetMemory(5), 0);
		/*
		According to DOS K3.3 Reference (N:SC18-2194-1)
		Byte 0: Type of Monitor
			Bit 7: Reserved
			Bit 6: Reserved
			Bit 5: Graphics Support Available
			Bit 4: Font-24 Color Monitor
			Bit 3: Font-16 Color Monitor
			Bit 2: Font-16 Color Monitor (720 x 512 Color Graphics mode is not available)
			Bit 1: Font-24 Monochrome Monitor
			Bit 0: Font-16 Monochrome Monitor
		Byte 1: Type of Display Adapter
			Bit 2-7: Reserved
			Bit 1: Display Adapter for 55xx-S/T / Display Adapter for 5570-S0A
			Bit 0: Monochrome Monitor / Color Monitor
		Byte 2: Reserved
		Byte 3: Reserved
		Byte 4: Reserved
		*/
		mem_writeb(Real2Phys(dos.tables.dcb_disp) + 0x00, 0x10);
		mem_writeb(Real2Phys(dos.tables.dcb_disp) + 0x01, 0x02);
		mem_writeb(Real2Phys(dos.tables.dcb_disp) + 0x02, 0x00);
		mem_writeb(Real2Phys(dos.tables.dcb_disp) + 0x03, 0x00);
		mem_writeb(Real2Phys(dos.tables.dcb_disp) + 0x04, 0x00);
	}
