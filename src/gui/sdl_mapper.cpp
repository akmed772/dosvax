...
/*	{"`~","grave",KBD_grave},*/	
	...
	{"=+","equals",KBD_equals},{ "\\","jp_yen",KBD_yen },{ "\x1B","bspace",KBD_backspace },
};

static KeyBlock combo_1_jp[15] = {
	/*	{"`~","grave",KBD_grave},*/
		{"ESC","esc",KBD_esc},		{"1!","1",KBD_1},	{"2\"","2",KBD_2},
		{"3#","3",KBD_3},			{"4$","4",KBD_4},	{"5%","5",KBD_5},
		{"6&","6",KBD_6},			{"7'","7",KBD_7},	{"8(","8",KBD_8},
		{"9)","9",KBD_9},			{"0~","0",KBD_0},	{"-=","minus",KBD_minus},
		{"^_","equals",KBD_equals},{ "\\|","jp_yen",KBD_yen },{ "\x1B","bspace",KBD_backspace },
};
...
static KeyBlock combo_4[12]={
	...
	{".","period",KBD_period},{ "/","slash",KBD_slash },{ "\\","jp_bckslash",KBD_backslash },
};
...
#define PY(_Y_) (10+(_Y_)*BH)
	//AddKeyButtonEvent(PX(0),PY(0),BW,BH,"ESC","esc",KBD_esc);
		...
		AddKeyButtonEvent(PX(4), PY(5), BW * 2, BH, "NCONV", "jp_muhenkan", KBD_nconv);//for AX
		AddKeyButtonEvent(PX(6), PY(5), BW * 3, BH, "SPACE", "space", KBD_space);
		AddKeyButtonEvent(PX(9), PY(5), BW * 2, BH, "CONV", "jp_henkan", KBD_conv);//for AX
...
#undef YO
	...
	else//for PS/55
	{
		...
		AddKeyButtonEvent(PX(0), PY(1), BW * 1, BH, "H/Z", "jp_hankaku", KBD_hankaku);
		for (i = 1; i < sizeof(combo_1_jp) / sizeof(combo_1_jp[0]); i++) AddKeyButtonEvent(PX(i), PY(1), BW, BH, combo_1_jp[i].title, combo_1_jp[i].entry, combo_1_jp[i].key);

		AddKeyButtonEvent(PX(0), PY(2), BW * 2, BH, "TAB", "tab", KBD_tab);
		for (i = 0; i < 12; i++) AddKeyButtonEvent(PX(2 + i), PY(2), BW, BH, combo_2_jp[i].title, combo_2_jp[i].entry, combo_2_jp[i].key);

		AddKeyButtonEvent(PX(14), PY(2), BW * 2, BH * 2, "ENTER", "enter", KBD_enter);

		caps_lock_event = AddKeyButtonEvent(PX(0), PY(3), BW * 2, BH, "CLCK", "capslock", KBD_capslock);
		for (i = 0; i < 12; i++) AddKeyButtonEvent(PX(2 + i), PY(3), BW, BH, combo_3_jp[i].title, combo_3_jp[i].entry, combo_3_jp[i].key);

		AddKeyButtonEvent(PX(0), PY(4), BW * 2, BH, "SHIFT", "lshift", KBD_leftshift);
		for (i = 1; i < sizeof(combo_4) / sizeof(combo_4[0]); i++) AddKeyButtonEvent(PX(2 + i), PY(4), BW, BH, combo_4[i].title, combo_4[i].entry, combo_4[i].key);
		AddKeyButtonEvent(PX(14), PY(4), BW * 2, BH, "SHIFT", "rshift", KBD_rightshift);

		/* Last Row */
		AddKeyButtonEvent(PX(0), PY(5), BW * 2, BH, "CTRL", "lctrl", KBD_leftctrl);
		AddKeyButtonEvent(PX(3), PY(5), BW * 1, BH, "KNJ", "lalt", KBD_leftalt);
		AddKeyButtonEvent(PX(4), PY(5), BW * 2, BH, "NCONV", "jp_muhenkan", KBD_nconv);//for AX
		AddKeyButtonEvent(PX(6), PY(5), BW * 3, BH, "SPACE", "space", KBD_space);
		AddKeyButtonEvent(PX(9), PY(5), BW * 2, BH, "CONV", "jp_henkan", KBD_conv);//for AX
		AddKeyButtonEvent(PX(11), PY(5), BW * 2, BH, "KANA", "jp_hiragana", KBD_kana);//for AX
		AddKeyButtonEvent(PX(13), PY(5), BW * 1, BH, "ALT", "ralt", KBD_rightalt);
		AddKeyButtonEvent(PX(14), PY(5), BW * 2, BH, "CTRL", "rctrl", KBD_rightctrl);
...
#define YO 7
		/* Numeric KeyPad */
		AddKeyButtonEvent(PX(XO), PY(YO), BW, BH, "*", "kp_multiply", KBD_kpmultiply);
		AddKeyButtonEvent(PX(XO + 1), PY(YO), BW, BH, "/", "kp_divide", KBD_kpdivide);
		AddKeyButtonEvent(PX(XO + 2), PY(YO), BW, BH, ",", "kp_commma", KBD_kpcomma);
		AddKeyButtonEvent(PX(XO + 3), PY(YO), BW, BH, "-", "kp_minus", KBD_kpminus);
		AddKeyButtonEvent(PX(XO + 0), PY(YO + 1), BW, BH, "7", "kp_7", KBD_kp7);
		AddKeyButtonEvent(PX(XO + 1), PY(YO + 1), BW, BH, "8", "kp_8", KBD_kp8);
		AddKeyButtonEvent(PX(XO + 2), PY(YO + 1), BW, BH, "9", "kp_9", KBD_kp9);
